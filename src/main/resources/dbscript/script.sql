...
DROP TABLE TB_ACCEPTED_KEYWORD CASCADE CONSTRAINTS;
...
DROP TABLE TB_EMOTION CASCADE CONSTRAINTS;
...
-- 합격 키워드
CREATE TABLE TB_ACCEPTED_KEYWORD (
                                     KEYWORD_NO NUMBER,
                                     JOB_CATEGORY VARCHAR2(100) NOT NULL,
                                     ACCEPT_KEYWORD VARCHAR2(100) NOT NULL,
                                     FREQUENCY NUMBER NOT NULL,
                                     REFERENCE_DATE DATE NOT NULL,
                                     CONSTRAINT ACCEPTED_PK_KEYWORD_NO PRIMARY KEY (KEYWORD_NO)
);

COMMENT ON COLUMN TB_ACCEPTED_KEYWORD.KEYWORD_NO IS '합격 키워드 번호';
COMMENT ON COLUMN TB_ACCEPTED_KEYWORD.JOB_CATEGORY IS '직무 카테고리';
COMMENT ON COLUMN TB_ACCEPTED_KEYWORD.ACCEPT_KEYWORD IS '합격 키워드';
COMMENT ON COLUMN TB_ACCEPTED_KEYWORD.FREQUENCY IS '사용빈도';
COMMENT ON COLUMN TB_ACCEPTED_KEYWORD.REFERENCE_DATE IS '기준 일자';
...
CREATE TABLE TB_VIDEO (
                          ...
                          CONSTRAINT VIDEO_FK_ITV_NO FOREIGN KEY (ITV_NO) REFERENCES TB_INTERVIEW(ITV_NO)  ON  DELETE CASCADE,
                          CONSTRAINT CH_ANSWER_NO CHECK (ANSWER_NO BETWEEN 1 AND 10),
                          CONSTRAINT CH_RESULT_DATA CHECK (RESULT_DATA BETWEEN 1 AND 100),
                          CONSTRAINT CH_ITV_TYPE CHECK (ITV_TYPE IN ('ANS', 'POS', 'EYE'))
);
...
CREATE TABLE TB_EMOTION (
                            EMOTION_NO NUMBER,
                            ITV_NO NUMBER NOT NULL,
                            ANSWER_ENO NUMBER NOT NULL,
                            ERESULT_DATA NUMBER NOT NULL,
                            EMOTION_NAME VARCHAR2(5) NOT NULL,
                            CONSTRAINT PK_EMOTION_DATA PRIMARY KEY (EMOTION_NO),
                            CONSTRAINT EMOTION_FK_ITV_NO FOREIGN KEY (ITV_NO) REFERENCES TB_INTERVIEW(ITV_NO) ON  DELETE CASCADE,
                            CONSTRAINT CH_ANSWER_ENO CHECK (ANSWER_ENO BETWEEN 1 AND 10),
                            CONSTRAINT CH_ERESULT_DATA CHECK (ERESULT_DATA BETWEEN 0 AND 101)
);
COMMENT ON COLUMN TB_EMOTION.EMOTION_NO  IS '비디오 번호';
COMMENT ON COLUMN TB_EMOTION.ITV_NO IS '면접 번호';
COMMENT ON COLUMN TB_EMOTION.ANSWER_ENO IS '답변 순서';
COMMENT ON COLUMN TB_EMOTION.ERESULT_DATA IS '결과 수치';
COMMENT ON COLUMN TB_EMOTION.EMOTION_NAME  IS '감정명';
...
CREATE TABLE TB_VOICE (
                          ...
                          CONSTRAINT VOICE_FK_ITV_NO FOREIGN KEY (ITV_NO) REFERENCES TB_INTERVIEW(ITV_NO) ON  DELETE CASCADE,
...
CREATE TABLE TB_CHATROOM_MEMBERS (
                                     ...
                                     CONSTRAINT FK_CHATROOM_MEMBERS_MSGNO FOREIGN KEY(MESSAGE_NO) REFERENCES TB_MESSAGES(MESSAGE_NO)
);

COMMENT ON COLUMN TB_CHATROOM_MEMBERS.CHATROOM_NO IS '채팅방번호';
COMMENT ON COLUMN TB_CHATROOM_MEMBERS.MEMBER_NO IS '채팅방 멤버 번호';
COMMENT ON COLUMN TB_CHATROOM_MEMBERS.MESSAGE_NO IS '마지막 채팅 번호';
...
CREATE SEQUENCE SEQ_C_REPORT_NO
    INCREMENT BY 1
    START WITH 1
    NOCACHE
NOCYCLE;
