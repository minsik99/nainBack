    ...
    public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        if (authorization != null && authorization.startsWith("Bearer ")) {
            String token = authorization.substring(7); // 'Bearer ' 문자 제거
                try {
                    log.info(token);
                    boolean tokenYN = jwtUtil.isTokenExpired(token);
                    log.info(tokenYN ? "token expired" : "token not expired");
            ...
            if (userOptional.isPresent()) {
                ...
                List<RefreshToken> refresh = refreshService.findMemberNo(member.getMemberNo());
                    ...
                    // 모든 리프레시 토큰을 삭제
                refresh.forEach(reToken -> refreshService.deleteByRefresh(reToken.getTokenValue()));
            }
        }
